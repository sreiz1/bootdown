...
def handleDivs(s, count, pageName, site_root, sister_sites):
    if (not "[." in s) and (not ".]" in s):
        return chef.cook(s, Environment(site_root, sister_sites))

    if (".]" in s) and (not "[." in s):
        if (count < 1):
            raise Exception("Mismatched divs, close without opening :: " + pageName + " :: " + s)
...
class BootDown:
    ...
    def make_menu(self):
        if "menu" in self.atts:
            ...
            self.atts["menu"] = '<ul class="nav navbar-nav">' + \
                "".join(link(x.strip()) for x in self.atts["menu"].split(",")) + '\n</ul>'
        else:
            self.atts["menu"] = ""
    ...
    def make_globals(self, s):
        ...
        self.make_menu()
        if not "head_extra" in self.atts:
            self.atts["head_extra"] = ""
...
if __name__ == '__main__':
    ...
    with open(os.path.join(cwd, fName)) as f:
        ...
        if "custom_template" in bd.atts:
            ...
            if "bootswatch" in bd.atts:
                # os.system("cp %sbs/bootswatches/%s/bootstrap.min.css %s/bs/css/" % (codeHome,bd.atts["bootswatch"],destPath))
                shutil.copy2(os.path.join(codeHome, "bs", "bootswatches",
                             bd.atts["bootswatch"], "bootstrap.min.css"), os.path.join(destPath, "bs", "css"))
